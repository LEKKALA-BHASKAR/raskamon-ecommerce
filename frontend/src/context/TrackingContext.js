...
export const TrackingProvider = ({ children }) => {
  ...
  // ── Callbacks defined first so useEffects below can reference them ────

  const fetchAbandonedCart = useCallback(async (userId) => {
    if (!userId) return;
    try {
      const { data } = await api.get(`/retargeting/abandoned-cart/${userId}`);
      if (data.hasAbandonedCart) setAbandonedCart(data.cart);
    } catch { /* ignore */ }
  }, []);
    ...
    const init = async () => {
      try {
        ...
        for (const pixel of data.pixels || []) {
          if (pixel.platform === 'facebook' && pixel.pixelId) injectFacebookPixel(pixel.pixelId);
          if (pixel.platform === 'gtm' && pixel.pixelId) injectGTM(pixel.pixelId);
        }
      } catch { /* non-fatal */ }
    };
    init();
  ...
  }, [user, fetchAbandonedCart, fetchNotifications, fetchRecommendations]);
