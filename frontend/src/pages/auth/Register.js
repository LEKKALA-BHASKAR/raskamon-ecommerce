...
const RoleSwitcher = ({ active = 'b2c' }) => {
  const roles = [
    { id: 'b2c', label: 'Retail Customer', desc: 'Shop wellness products', icon: User, to: '/register' },
    { id: 'b2b', label: 'Business Buyer', desc: 'Wholesale pricing + GST', icon: Briefcase, to: '/register/b2b' },
    { id: 'vendor', label: 'Vendor / Seller', desc: 'Sell on Dr MediScie', icon: Store, to: '/register/vendor' },
  ];
  return (
    <div data-testid="register-role-switcher" className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-6">
      {roles.map((r) => {
        const Icon = r.icon;
        ...
        return (
          <Link
            ...
            className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
              isActive
                ? 'border-[var(--sattva-forest)] bg-[var(--sattva-forest)] text-[var(--sattva-cream)] shadow-md'
                : 'border-[color:var(--sattva-border)] bg-[var(--sattva-surface)] text-[var(--sattva-ink)] hover:border-[var(--sattva-gold)]'
            }`}
          >
            <div className={`p-1.5 rounded-lg ${isActive ? 'bg-[rgba(250,248,245,0.15)]' : 'bg-[var(--sattva-muted)]'}`}>
              <Icon size={16} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold leading-tight">{r.label}</p>
              <p className={`text-[10px] leading-tight mt-0.5 ${isActive ? 'opacity-80' : 'text-gray-500'}`}>{r.desc}</p>
            </div>
      ...
      <div className="min-h-[calc(100vh-72px)] flex items-center justify-center py-12 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xl">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm mt-1">Choose the type of account that fits you</p>
          </div>

          <RoleSwitcher active="b2c" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                ...
                <div key={field.key}>
                  ...
                  <input
                    data-testid={`register-b2c-${field.key}`}
                    type={field.type}
                    {...f(field.key)}
              ...
              <button
                data-testid="register-b2c-submit"
                type="submit"
                disabled={loading}
